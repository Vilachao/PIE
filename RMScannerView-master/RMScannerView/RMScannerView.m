...
- (void)startCaptureSession {
    ...
    if (videoInput) {
        ...
        if ([self.captureSession.inputs containsObject:videoInput] == NO)
...
//        
//        if(laserView!=nil) laserView.alpha =0;
//        laserView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 2)];
...
{
    //(AVCaptureVideoOrientation)[[UIDevice currentDevice] orientation] esta linea es para que automaticamente se ajuste
    if (previewLayer) {
